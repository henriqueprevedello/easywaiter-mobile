...
export class ListaEstabelecimentoPage implements OnInit {
  ...
  adquirirEstabelecimentos() {
    ...
    this.estabelecimentos = this.estabelecimentosBuscados;
  }

  filtrar(pesquisa: any) {
    this.estabelecimentos = this.estabelecimentosBuscados.filter((cidade) =>
      cidade.name.toLowerCase().includes(pesquisa.target.value.toLowerCase())
    );
  }
